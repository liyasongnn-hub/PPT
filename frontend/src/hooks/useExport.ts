...
export default () => {
      ...
      await new Promise<void>((resolve) => {
        setTimeout(() => {
          pptx.writeFile({ fileName: `${title.value}.pptx` })
            .then(() => resolve())
            .catch(() => { message.error('导出失败'); resolve() })
        }, 200)
      })
    } finally {
      exporting.value = false
  ...
  // 保存到云端：导出为 blob 并上传到项目 exports（对象存储）
  const exportPPTXToCloud = async (projectId: string, _slides: Slide[], masterOverwrite: boolean, ignoreMedia: boolean) => {
    exporting.value = true
    try {
      const pptx = await buildPPTX(_slides, masterOverwrite, ignoreMedia)
      const blob = await pptx.write({ outputType: 'blob' }) as Blob
      const file = new File([blob], `${title.value}.pptx`, {
        type: 'application/vnd.openxmlformats-officedocument.presentationml.presentation',
      })
      const record = await api.uploadExport(projectId, file)
