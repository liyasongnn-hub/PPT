<template>
  <div class="export-pptx-dialog">
    ...
    <div class="btns">
      ...
      <Button class="btn cloud" @click="saveToCloud">保存到云端</Button>
...
const { exportPPTX, exportPPTXToCloud, exporting } = useExport()
...
const saveToCloud = async () => {
  if (!projectId.value) {
    message.error('缺少项目信息，无法保存到云端')
    return
  }
  try {
    await exportPPTXToCloud(projectId.value, selectedSlides.value, masterOverwrite.value, ignoreMedia.value)
  } catch {
    // 错误提示已在 hook 内完成
  }
}
