...
const execAI = async (command: string) => {
  ...
  try {
    ...
    if (!stream.ok) {
      const detail = await stream.text()
      throw new Error(detail || `AI 改写服务返回 ${stream.status}`)
    }
    if (!stream.body) throw new Error('AI 改写服务没有返回内容')
    ...
    message.error(error?.message || 'AI 改写失败，请稍后重试')
  } finally {
