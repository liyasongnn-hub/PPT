...
class ProductionStarter:
    def __init__(self):
        ...
        # npm 在 Windows 上是 npm.cmd，直接 Popen(['npm', ...]) 会报 WinError 2
        self.npm_executable = shutil.which('npm.cmd' if os.name == 'nt' else 'npm') or ('npm.cmd' if os.name == 'nt' else 'npm')
...
{'='*80}
    ...
    def install_dependencies(self):
        """按需安装依赖，避免每次启动都修改全局 Python 环境。"""
        if os.environ.get('INSTALL_DEPENDENCIES', '').lower() not in {'1', 'true', 'yes'}:
            self.logger.info("跳过依赖安装；如需更新依赖，请设置 INSTALL_DEPENDENCIES=true")
            return
        ...
        if package_json.exists():
            ...
            subprocess.run([self.npm_executable, 'install'], cwd=self.frontend_dir, check=True)
