...
# 每进程复用同一个 PersistentClient，避免每个请求都重新打开 Chroma 数据库。
_client_cache: Dict[str, Any] = {}
_client_lock = threading.Lock()


def _get_persistent_client(db_dir: str):
    with _client_lock:
        client = _client_cache.get(db_dir)
        if client is None:
            client = chromadb.PersistentClient(path=db_dir, settings=Settings(anonymized_telemetry=False))
            _client_cache[db_dir] = client
        return client
...
def cache_decorator(func):
    ...
    def wrapper(*args, **kwargs):
        # Cached values can contain extracted user documents and embeddings.
        # Keep caching opt-in and never log the raw cache key.
        cache_enabled = os.environ.get("PERSONALDB_CACHE_ENABLED", "false").strip().lower() in {"1", "true", "yes", "on"}
        if not cache_enabled:
            kwargs.pop("usecache", None)
            return func(*args, **kwargs)
        cache_path = os.path.abspath(os.environ.get("PERSONALDB_CACHE_DIR", "cache"))
        ...
        cache_id = cal_md5(namespace + "|" + key)
        key_file = os.path.join(cache_path, cache_id + "_cache.pkl")
        # 如果结果已缓存，则返回缓存的结果
        if os.path.exists(key_file) and usecache:
            # 去掉kwargs中的usecache
            logger.info("%s 缓存命中 cache_id=%s", func.__name__, cache_id[:12])
            try:
                with open(key_file, 'rb') as f:
                    result = pickle.load(f)
                    return result
            except Exception as e:
                logger.warning("%s 缓存读取失败 cache_id=%s: %s", func.__name__, cache_id[:12], e)
        result = func(*args, **kwargs)
        # 将结果缓存到文件中
        # 如果返回的数据是一个元祖，并且第1个参数是False,说明这个函数报错了，那么就不缓存了，这是我们自己的一个设定
        if isinstance(result, tuple) and result[0] == False:
            logger.info("%s 返回失败状态，不写缓存", func.__name__)
        else:
            with open(key_file, 'wb') as f:
                pickle.dump(result, f)
            logger.info("%s 已写入缓存 cache_id=%s", func.__name__, cache_id[:12])
        return result
...
class ChromaDB(object):
    def __init__(self, embedder, db_dir=None):
        """
        Args:
        ...
        db_dir = os.path.abspath(db_dir or os.environ.get("CHROMA_DB_DIR", "cache/chromadb"))
        if not os.path.exists(db_dir):
            os.makedirs(db_dir)
        self.client = _get_persistent_client(db_dir)

    def _cosine_collection(self, name: str):
    ...
    def delete_one_document(self, collection, doc_id):
        ...
        """
        try:
            col = self._cosine_collection(collection)
            # 删除指定 ID 的文档
    ...
    def delete_file_vectors(self, user_id: int, file_id: int):
        ...
        try:
            ...
            col = self._cosine_collection(collection_name)
...
class EmbeddingModel(object):
    ...
    def do_embedding(self, texts: List[str]):
        ...
        if failures:
            raise RuntimeError(f"{len(failures)} 个向量批次生成失败")
        if len(result["data"]) != len(texts):
            raise RuntimeError("向量服务返回数量与输入文本数量不一致")
