...
**访问地址：**
- 前端统一入口（登录/项目工作台）：`http://127.0.0.1:5173`
- 主 API 文档：`http://127.0.0.1:6800/docs`

### 产品化 MVP 能力

主 API 现在提供 `/api/v1` 工作流：注册/登录（JWT）、项目创建与持久化、生成任务状态、事件 SSE、取消任务和质量检查。数据层基于 SQLAlchemy 2.0，本地开发未配置 `DATABASE_URL` 时自动回退 SQLite（`data/platform.db`）；生产配置 `DATABASE_URL` 后自动切换 PostgreSQL，任务队列切换 Redis Stream，文件交付切换 MinIO，无需改代码。原有 `/tools/*` 接口保持兼容。
...
#### 方法四：Docker Compose 部署（确保能连接 Docker Hub 网络）

- `docker-compose.yml` **仅用于本地开发**：映射后端端口（9100/10001/10011/6800）并挂载源码热更新。
- **生产部署请使用 `docker-compose.prod.yml`**：PostgreSQL + Redis + MinIO + worker/scheduler/backup，仅前端暴露端口，详见 [生产环境部署指南](README_PRODUCTION.md)。
